...
public partial class Search : System.Web.UI.Page
{
    ...
    private SearchProviderCollection _providers = null;
    ...
    private void SoapSearch(SourceType sourceType, string query, int first, int count, string domain)
    {
        ...
        if (_provider == null)
        {
            SearchProviderSection section = (SearchProviderSection)
                WebConfigurationManager.GetSection("system.web/searchService");
            _providers = new SearchProviderCollection();
            ProvidersHelper.InstantiateProviders(section.Providers, _providers, typeof(SearchProvider));
        ...
        try
        {
            if (_provider == null)
            {
                ...
                result = CreateXmlSearchResults(sourceType, searchResponse.Responses[0], domain);
            }
            else if (_provider is SearchProviderSharePoint)
            {
                SearchProviderSharePoint sharePointProvider = new SearchProviderSharePoint(query);
