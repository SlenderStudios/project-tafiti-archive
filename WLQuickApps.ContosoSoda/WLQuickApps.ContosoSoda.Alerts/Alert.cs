...
using WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription;
using WLQuickApps.ContosoSoda.Alerts;
using RecAlertsRequestResponse = WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecAlertsRequestResponse;
using RecServicesContact = WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Message.RecServicesContact;
using RecServicesHeader = WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Message.RecServicesHeader;
using RecServicesIdentification = WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Message.RecServicesIdentification;
using RecServicesRequestResponse = WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesRequestResponse;

namespace WLDemo.Alerts
{
    public class Alert
    {
        ...
        public static void SendGroupRequest(RecServicesGroupMessage message, string sendtoTransport)
        {
            ...
            WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Message.RecServicesRequestResponse response =
        ...
        private static WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesIdentification createRequestID()
        {
            WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesIdentification recID =
                new WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesIdentification();
            recID.PINID = Convert.ToInt32(ConfigurationManager.AppSettings["AlertPIN"]);
            recID.PW = ConfigurationManager.AppSettings["AlertPassword"];
            return recID;
        }

        private static WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesHeader createRequestHeader()
        {
            string messageID = Guid.NewGuid().ToString();
            WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesHeader recHeader = new WLQuickApps.ContosoSoda.Alerts.Live.Alerts.Subscription.RecServicesHeader();
            recHeader.version = "1.0";
            DateTime now = DateTime.Now;
            recHeader.timestamp = String.Format("{0}T{1}", now.ToString("yyyy-MM-dd"), now.ToString("HH:mm:sszzz"));
        ...
        {
            AlertsWebServicesService alertsService = new AlertsWebServicesService();
            RecServicesRequestResponse response = alertsService.AddGroup(createRequestHeader(), createRequestID(), "group 1", "group 1");
        }
